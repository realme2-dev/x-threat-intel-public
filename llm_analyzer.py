...
아래 뉴스/트윗 데이터에 원본 링크가 있는 내용을 언급할 때는 문장 끝에
`([출처명](링크))` 형식으로 근거 링크를 반드시 붙이세요. 링크를 확인할 수 없는
일반론이나 여러 출처를 종합한 서술에는 링크를 억지로 붙이지 마세요.

### 1. 현재 주요 사이버보안 트렌드
(3~4문장. 이번 수집에서 두드러지는 공격 벡터/기술 변화 위주. 특정 기사/트윗에
기반한 문장에는 `([출처명](링크))`를 붙일 것)

### 2. 주요 위협 행위자 / 캠페인
- 행위자명 (출처 근거, 가능하면 링크 포함): 활동 내용 및 타겟
- (확인된 항목만, 최대 5개)

### 3. 한국 관련 위협
- 한국 기업·기관·인프라 관련 언급 항목 (출처 링크 포함)
- 한국어 트윗([KR])에서 감지된 주요 사건 (해당 트윗 링크 포함)
- (없으면 "이번 수집에서 한국 특정 위협 없음")

### 4. 주요 CVE / 취약점 (CVSS 우선)
- CVE-ID: 영향 제품, 심각도, 현재 악용 여부 ([출처명](링크))
- (최대 5개, CVSS 높은 순)
